...
export class Heartbeat {
  ...
  rppg() {
    ...
    if (this.signal.length >= this.targetFps * this.windowSize) {
      ...
      // console.log(signal) // rPPG rPPG rPPG rPPG rPPG rPPG rPPG rPPG rPPG 
  ...
  ENES(signal){
    ...
    if(frequencyCounter==20){
      $.ajax({
        url: "http://127.0.0.1:5000/enes",
        type:"POST",
        //dataType:"json",
        ...
        success: function (response) {
            // alert(response)
            $("#dataOne").html(response);
            
            // $("#dataOne").html(response.data2);
        }
    });
      console.log("SIGNAL=" + newList)
      console.log("\n")
  ...
  drawTime(signal) {
    ...
    if(isPostData==false&&frequencyCounter>50){
      ...
      // $.ajax({
      //   url: "url",
      //   success: function(response){ response type json -> returned response response=10 response{
       // data1:10,
        //data2:20
        // response[0]
        // response[1]//if its an array
