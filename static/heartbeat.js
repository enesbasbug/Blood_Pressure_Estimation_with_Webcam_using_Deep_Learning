 ...
 //     if (!this.classifier.load(faceCascadeFile)) {
  ...
  rppg() {
    ...
    if (this.signal.length >= this.targetFps * this.windowSize) {
      ...
      console.log("SINYAL LENGTH," + this.signal.length)
      ...
      // console.log(signal) // CELAL HOCA BURASI OLABILIR DEDI rPPG rPPG rPPG rPPG rPPG rPPG rPPG rPPG rPPG rPPG 
      ...
      // this.drawTime(signal); // USTTEKI -___USTTEKI -___USTTEKI -___USTTEKI -___USTTEKI -___USTTEKI -___
      ...
      if (!signal.empty()) {
        ...
        // this.drawFrequency(signal, low, high, bandMask); // ALLTA__ALLTA__ALLTA__ALLTA__ALLTA__ALLTA__ALLTA__
  ...
  ENES(signal){
    ...
    for (var i = 1; i < signal.rows; i++) {
      ...
      if (i < 181) {
        ...
        ekle.push(signal.data32F[i]);

      }

    }

    var avg = ekle.reduce((a, b) => a + b) / ekle.length;
    var std = Math.sqrt(ekle.map(x => Math.pow(x - avg, 2)).reduce((a, b) => a + b) / ekle.length);
    var newList = ekle.map(function (num) {
      return (num - avg) / std;
    })
    //Math.sqrt(list.map(x => Math.pow(x - avg, 2)).reduce((a, b) => a + b) / list.length)
    console.log("BANA BAK" + newList + "\n");
  ...
  // BURAYA BI BAK BURAYA BI BAK BURAYA BI BAK BURAYA BI BAK BURAYA BI BAK BURAYA BI BAK BURAYA BI BAK BURAYA BI BAK BURAYA BI BAK BURAYA BI BAK BURAYA BI BAK BURAYA BI BAK 
  ...
  selectGreen(signal) {
    ...
    let result = rgb.get(1); // ONCEKINDE BI BU VARDI.
  ...
  drawTime(signal) {
    ...
    var avg = bune.reduce((a, b) => a + b) / bune.length;
    var std = Math.sqrt(bune.map(x => Math.pow(x - avg, 2)).reduce((a, b) => a + b) / bune.length);
    var newList = bune.map(function (num) {
      return (num - avg) / std;
    })
    //Math.sqrt(list.map(x => Math.pow(x - avg, 2)).reduce((a, b) => a + b) / list.length)
    ...
    console.log('-------');
    console.log('\n');
    //frequencycounter 250ms de bir artyor yani 50 icin 12.5 sn de gircek
    if(isPostData==false&&frequencyCounter>50){
      //veriyi yolla

      // $.ajax({
      //   url: "url",
      //   success: function(response){ response tipi json -> Burada veri dönmğş hali response response=10 response{
       // data1:10,
        //data2:20
        // response[0]
        // response[1]//array ise eger
      //}console.log(response)
      //     $("#dataOne").html(response.data2);
      //   }
      // });
      isPostData=true;
    }
